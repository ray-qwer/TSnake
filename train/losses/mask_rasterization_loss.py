...
# Copy files from: copy from https://github.com/zhang-tao-whu/e2ec_mmdet/blob/master/mmdet/models/losses/contour_loss.py
# It is called for the loss from "AttentiveContourEvolveHead"
import torch
...
from .utils import cropnResize, MaskCropnResize, get_resize_transforms
...
class MaskRasterizationLoss(nn.Module):
    # initial: loss_weight: 0.33, resolution:[64,64], inv_smoothness:0.1
    def __init__(self, resolution=(64, 64), inv_smoothness=0.1):
        ...
        self.name = "mask"
        self.transform = get_resize_transforms(resolution)
    ...
    def crop_targets_masks_from_poly(self, targets_polys, union_bboxes):
        ret = []
        # sx = 0
        for idx, targets_poly in enumerate(targets_polys):
            # check if it is per batch or per polygon
            ret.append(torch.tensor(cropnResize(targets_poly, union_bboxes[idx], self.resolution), dtype=torch.float32, device=union_bboxes.device))

        return torch.cat(ret, dim=0)

    def crop_targets_masks_from_mask(self, targets_masks, union_bboxes):
        assert len(targets_masks.shape) == 3
        ret = []
        for idx, (targets_mask, union_bbox) in enumerate(zip(targets_masks, union_bboxes)):
            ret.append(MaskCropnResize(targets_mask, union_bbox, self.transform))
        return torch.cat(ret, dim=0)
    ...
    def forward(self, preds, targets, targets_bboxes, avg_factor=None, is_gt_mask=False):
        # targets -> if is_gt_mask: targets is masks, else: targets is polys
        ...
        if is_gt_mask:
            input_h, input_w = targets.shape[-2:]
            union_bboxes[:,[0,2]] = union_bboxes[:,[0,2]].clamp(0, input_w-1)
            union_bboxes[:,[1,3]] = union_bboxes[:,[1,3]].clamp(0, input_h-1)
            targets_masks = self.crop_targets_masks_from_mask(targets, union_bboxes)
        else:
            targets_masks = self.crop_targets_masks_from_poly(targets, union_bboxes)

        resolution = self.rasterize_at[0]

        # -0.5 needed to align the rasterizer with COCO.
        preds = self.get_normed_polygons(preds, union_bboxes)       # normalize
        ...
        return self.loss_fn(pred_masks, targets_masks, avg_factor=avg_factor)