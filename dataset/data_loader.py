## USELESS NOW, THE MODIFIED COULD BE CHANGED AT .BASE.PY
from .train import cityscapes_mask
import torch
...
def make_dataset(dataset_name, is_test, cfg):
    ...
    if is_test:
        from .test import coco, cityscapes, cityscapesCoco, sbd, kitti, cityscapes_test
        dataset_dict = {'coco': coco.CocoTestDataset, 'cityscapes': cityscapes.Dataset, 'cityscapes_test': cityscapes_test.Dataset,
        ...
        from .train import coco, cityscapes, cityscapesCoco, sbd, kitti, cityscapes_mask
        dataset_dict = {'coco': coco.CocoDataset, 'cityscapes': cityscapes.CityscapesDataset,
                        'cityscapesCoco': cityscapesCoco.CityscapesCocoDataset, 
                        'cityscapes_mask': cityscapes_mask.CityscapesMaskDataset,
...
def make_train_loader(cfg):
    ...
    shuffle = cfg.train.shuffle     # shuffle is True
    drop_last = False
    dataset_name = cfg.train.dataset
    print(batch_size, shuffle, dataset_name)
...
def make_demo_loader(data_root=None, cfg=None, sample_num = 0):
    from .demo_dataset import Dataset
    batch_size = 1
    shuffle = False
    drop_last = False
    dataset = Dataset(data_root, cfg, sample_num = sample_num)
    sampler = make_data_sampler(dataset, shuffle)
    batch_sampler = make_batch_data_sampler(sampler, batch_size, drop_last)
    num_workers = 1
    collator = collate_batch
