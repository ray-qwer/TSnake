...
class Dataset(data.Dataset):
    def __init__(self, data_root, cfg, sample_num=0):
        super(Dataset, self).__init__()
        self.data_root = data_root
        self.split = 'test'
        self.cfg = cfg
        self.image_names, self.paths = self.process_info(data_root, sample_num)

    def process_info(self, data_root, sample_num):
        image_name = os.listdir(data_root)
        if sample_num > 0:
            import random
            random.seed(42)
            image_name = random.sample(image_name, sample_num)
