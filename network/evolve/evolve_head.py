...
# copy from https://github.com/zhang-tao-whu/e2ec_mmdet/blob/master/mmdet/models/dense_heads/contour_head.py
...
def interpolation(poly, time=10):
    ...
    poly_ = poly.unsqueeze(3).repeat(1, 1, 1, time) # (N, P, 2, 10)
...
class PointResampler:
    ...
    def __call__(self, polys, sample_ratio=None):
        ...
        interpolated_polys = interpolation(polys, time=self.density)    # (N,P*density,2)
        ...
        if self.mode == 'align_uniform':
            ...
            cum_lengths = self.get_cum_lengths(interpolated_polys) # accumulated length
            ...
            # NOTE: here, cannot understand, its alignment is just the alignment of index but not direction -> check its shape
...
class FPNContourProposalHead(BaseModule, metaclass=ABCMeta):
    ...
    def forward(self, ms_feats, gt_centers, gt_whs, img_h, img_w, inds):
        ...
        if self.use_tanh[0]:
            shape_embed = shape_embed.tanh()        # normalized to [-1,1] then multiply with the bbox w,h; This is default usage
            contours_proposal = gt_centers.detach().unsqueeze(1) + shape_embed * gt_whs.unsqueeze(1)
        else:
            contours_proposal = gt_centers.detach().unsqueeze(1) + shape_embed * strides    # the same with e2ec, it predicts a contour, which is 1/stride size
        contours_proposal_ret = contours_proposal

        contours_proposal = self.sampler(contours_proposal) # NOTE: what do sampler do?
        ...
        # NOTE: b*n*32*2, b*n*64*2, after samplers, normalized init offset, normalized coarse offset, stride
    def compute_contour_losses(self, normed_init_offset_pred, normed_global_offset_pred,
                               normed_init_offset_target, normed_global_offset_target):
        num_poly = torch.tensor(
            len(normed_init_offset_target), dtype=torch.float, device=normed_init_offset_pred.device)
        ...
        return dict(loss_init_contour=loss_init,        
                    loss_coarse_contour=loss_coarse)    # smooth-l1-loss
...
@HEADS.register_module()
class IamFPNContourProposalHead(FPNContourProposalHead):
    ...
    def forward(self, ms_feats, gt_centers, gt_whs, img_h, img_w, inds):
        ...
        instances_features = self.extract_features_instance(ms_feats, gt_centers, gt_whs,
                                                            img_h, img_w, inds, ms_inds) # (n, c, r, r)
...
class BaseContourEvolveHead(BaseModule, metaclass=ABCMeta):
    ...
    def forward_train(self,
                      x,
                      img_metas,
                      contour_proposals,    # where it from?
                      gt_contours,
                      inds,
                      key_points=None,
...
class DeformAttentiveContourEvolveHead(AttentiveContourEvolveHead):
    ...
    def prepare_memory(self, feats_):
        ...
        for i in range(len(feats)):
            ...
            pos_embed = pos_embed.unsqueeze(2).repeat(1, 1, ori_batch_size, 1, 1).flatten(2, 3)
